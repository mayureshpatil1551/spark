...
- The operation only minimizes data movement by merginh into fewer partitions on the same executor. It doesnt do any shuffling..
- because no shuffling is done. the partitioning sheme remains the same as the original DF and spark doesn't include it explicity in it
